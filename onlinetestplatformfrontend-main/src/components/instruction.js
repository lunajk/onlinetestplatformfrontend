        ...
        detectionInterval = setInterval(async () => {
            ...
            try {
                ...
                if (validDetections.length > 0) {
                    ...
                    const isValid = validateFaceOrientation(validDetections[0].landmarks);
                    setCaptureStatus(isValid ? 'Looking straight ✅' : 'Please look straight at the camera');
                } else {
                    setCaptureStatus('No valid face detected');
                }
            } catch (error) {
...
if (validDetections.length === 0) throw new Error('No face detected');
if (validDetections.length > 1) throw new Error('Multiple faces detected');
    ...
    const handleStartTest = () => {
        ...
        .then(() => {
            console.log("Navigating to:", randomString, testId);
            navigate(`/smartbridge/online-test-assessment/${uuid}/write`);
        })
        .catch(err => {
